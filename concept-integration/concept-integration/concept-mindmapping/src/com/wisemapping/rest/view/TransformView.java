...
public class TransformView extends AbstractView {
	...
	@Override
	protected void renderMergedOutputModel(@NotNull Map<String, Object> viewMap, @NotNull HttpServletRequest request,
		...
		// Set file
		// name...:http://stackoverflow.com/questions/5325322/java-servlet-download-filename-special-characters/13359949#13359949
		...
		try {
			...
			} else if (exportFormat == ExportFormat.MICROSOFT_EXCEL || exportFormat == ExportFormat.TEXT
					|| exportFormat == ExportFormat.OPEN_OFFICE_WRITER || exportFormat == ExportFormat.MINDJET) {
				response.setCharacterEncoding(DEFAULT_ENCODING);
				factory.export(properties, content, outputStream, null);
			} else {
				// Image export ...
